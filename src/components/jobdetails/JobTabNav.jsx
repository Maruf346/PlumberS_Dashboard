...
function IconNoteCalendar() { return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="1.5" y="3" width="13" height="11.5" rx="1.5" stroke="currentColor" strokeWidth="1.2"/><path d="M5.5 1.5v3M10.5 1.5v3M1.5 7h13" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/><path d="M5 10h2M9 10h2M5 12.5h2" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg> }
...
const TABS = [
  ...
  { id: 'schedulenotes', label: 'Notes',        Icon: IconNoteCalendar },
]

export default function JobTabNav({ activeTab, onTabChange, notesCount }) {
  return (
      ...
      {TABS.map(({ id, label, Icon }) => {
        ...
        return (
          <button key={id} onClick={() => onTabChange(id)}
            ...
            <Icon />
            {label}
            {id === 'schedulenotes' && notesCount > 0 && (
              <span className={`ml-0.5 inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-full text-[10px] font-bold ${isActive ? 'bg-[#f54900]/15 text-[#f54900]' : 'bg-[#f1f5f9] text-[#62748e]'}`}>
                {notesCount}
              </span>
            )}
          </button>
