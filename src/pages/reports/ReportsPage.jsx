...
// Reports Manager — /admin/reports
//
// API endpoints wired:
//   GET  custom-reports/field-types/                       → populate type dropdown
//   GET  custom-reports/?all=true                          → list all templates (incl. inactive)
//   GET  custom-reports/{id}/                              → load selected template + fields
//   POST custom-reports/create/                            → create template
//   PATCH custom-reports/{id}/update/                      → update template metadata (Save Changes)
//   DELETE custom-reports/{id}/update/                     → delete template
//   POST custom-reports/{template_id}/fields/add/          → add field (returns real ID)
//   PATCH custom-reports/{template_id}/fields/{field_id}/  → update field (called on input blur)
//   DELETE custom-reports/{template_id}/fields/{field_id}/ → delete field
//   POST custom-reports/{template_id}/fields/reorder/      → reorder after drag-drop
//
...
// Create Template Modal — wired to POST /custom-reports/create/
function CreateTemplateModal({ onClose, onCreate }) {
  ...
  const handleCreate = async () => {
    if (!validate()) return
    setSaving(true)
    const { data, ok } = await apiFetch('custom-reports/create/', {
      method: 'POST',
      body: JSON.stringify({
        name:        form.name.trim(),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0f172b]/40 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-[460px] bg-white rounded-[16px] shadow-[0px_20px_60px_rgba(15,23,43,0.22)] overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#f1f5f9]">
          <div>
            <h2 className="text-[#0f172b] font-bold text-[18px]">New Report Template</h2>
            <p className="text-[#90a1b9] text-[13px] mt-0.5">Create a blank report template</p>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0f172b]/40 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-[540px] max-h-[80vh] bg-white rounded-[16px] shadow-[0px_20px_60px_rgba(15,23,43,0.22)] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#f1f5f9]">
          <div>
            <p className="text-[#90a1b9] text-[11px] font-semibold uppercase tracking-[0.5px] mb-1">Report Preview</p>
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    ;(async () => {
      const { data, ok } = await apiFetch('custom-reports/field-types/')
  ...
  const fetchTemplates = useCallback(async ({ preserveId = null, autoSelectFirst = false } = {}) => {
    ...
    const { data, ok } = await apiFetch('custom-reports/?all=true')
    if (ok && data) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setSelected(null)
      const { data, ok } = await apiFetch(`custom-reports/${selectedId}/`)
  ...
  const handleSave = async () => {
    if (!selected) return
    setSaving(true)
    const { ok } = await apiFetch(`custom-reports/${selected.id}/update/`, {
      method: 'PATCH',
      body: JSON.stringify({
        name:        selected.name,
  ...
  const handleDeleteTemplate = async () => {
    ...
    const { ok } = await apiFetch(`custom-reports/${deleteTemplateTarget.id}/update/`, { method: 'DELETE' })
    if (ok) {
  ...
  const addField = async () => {
    ...
    const { data, ok } = await apiFetch(`custom-reports/${templateId}/fields/add/`, {
      method: 'POST',
  ...
  const handleFieldBlur = useCallback(async (fieldState) => {
    ...
    await apiFetch(`custom-reports/${templateId}/fields/${fieldState.id}/`, {
      method: 'PATCH',
      body: JSON.stringify({
  ...
  const deleteField = useCallback(async (fieldId) => {
    ...
    const { ok } = await apiFetch(`custom-reports/${templateId}/fields/${fieldId}/`, { method: 'DELETE' })
    if (ok) {
  ...
  const handleDrop      = async (dropIndex) => {
    ...
    setReordering(true)
    await apiFetch(`custom-reports/${templateId}/fields/reorder/`, {
      method: 'POST',
    ...
    <>
      ...
      <div className="p-6 lg:p-8 flex flex-col gap-6 min-h-full">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-[#0f172b] font-bold text-[26px] leading-[34px]">Reports Manager</h1>
            <p className="text-[#62748e] text-[14px] mt-1">Configure custom report templates and fields</p>
          </div>
        ...
        <div className="flex gap-5 flex-1 min-h-0 items-start">
          ...
          <div className="w-[300px] shrink-0 bg-white border border-[#e2e8f0] rounded-[14px] overflow-hidden shadow-[0px_1px_3px_rgba(0,0,0,0.07)]">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#f1f5f9]">
              <h2 className="text-[#0f172b] font-bold text-[15px]">Report Templates</h2>
          ...
          {templateLoading ? (
            ...
            <div className="flex-1 min-w-0 bg-white border border-[#e2e8f0] rounded-[14px] overflow-hidden shadow-[0px_1px_3px_rgba(0,0,0,0.07)]">
              <div className="px-6 py-5 flex flex-col gap-5">
                ...
                {/* Report title + Active toggle — unchanged layout */}
                <div className="flex items-start justify-between gap-4">
                  <div className="flex flex-col gap-1 flex-1 min-w-0">
                    <span className="text-[#90a1b9] text-[11px] font-bold uppercase tracking-[0.6px]">Report Title</span>
